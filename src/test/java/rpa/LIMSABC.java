...
public class LIMSABC {
	Logger logger = LogManager.getLogger(LIMSABC.class);
...
String CPT2= ", 80307";
	...
	static String excelFileName="Toxicology Coding Approval LimsABC 01-25-2024 100.xlsx",fileName,excelFileName1= "Toxicology Drug classes. Updated (1).xlsx", accessionNum,renameStatus,downloadStatus, CPT="";
	...
	public  void pdfDownload(Hashtable<String,String> data) throws IOException, InterruptedException {
		renameRowNum++;
	 	...
	 	downloadStatus = data.get("Download Status");
	 	accessionNum= data.get("Accession No");
    	if(downloadStatus.isBlank()|| downloadStatus.isBlank() ) {	
    		Thread.sleep(3000);
    ...
    Thread.sleep(3000);
    	driver.findElement(By.xpath("//div[@title='View Report']")).click();
...
/*
	...
	public  void renameFile(Hashtable<String,String> data) throws IOException {
		screen=false;
		rowNum++;
    	accessionNum = data.get("Accession No");
renameStatus= data.get("Rename Status");
		if(renameStatus.isBlank()|| renameStatus.isBlank() && downloadStatus.equals("Pass")) {
		

			File folder = new File("C:\\Users\\jmartin\\eclipse-workspace\\TestSequence\\DownloadedFiles\\");
			File[] listOfFiles = folder.listFiles();

			for (int i = 0; i < listOfFiles.length; i++) {
			  if (listOfFiles[i].isFile()) {
			   
			    if(listOfFiles[i].getName().contains(accessionNum)) {
	...
	*/
	...
	public  void pdfParse(Hashtable<String,String> data) throws IOException, InterruptedException {
		screen=false;
		rowNum++;
    	accessionNum = data.get("Accession No");
status= data.get("Status");
		...
		String pdfFilePath = "C:\\Users\\jmartin\\eclipse-workspace\\TestSequence\\DownloadedFiles\\"+fileName;
			  ...
			  if(currentLine.contains("Screen")) {
                          ...
                          for(int j=2; j<excel1.getRowCount(sheetName); j++) {
                                  ...
                                  //        System.out.println(line1+"----------"+currentLine);
                                          mysetScreen.add(j);
                          screenArr.add(currentLine.split(" ")[0]);
...
//		System.out.println(myset1.size());
		...
		excel.setCellData(sheetName, "Coding", rowNum, CPT);
	...
	public static boolean isWordPresent(String str1, String str2) {
		...
		for (String word : words1) {
			...
			else if(containsIgnoreCase(words3, word.replace(",", "").trim())) {
				return true;
			}
